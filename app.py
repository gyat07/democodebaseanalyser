...
.topnav { display: flex; align-items: center; justify-content: space-between; gap: 16px; padding: 6px 0 8px; }
...
.hero { padding: 32px 0 20px; animation: rise 700ms var(--ease) both; }
.hero-badge {
...
.pulse { width: 7px; height: 7px; border-radius: 50%; background: var(--primary); animation: pulse 2.2s ease-out infinite; }
...
.hero-meta { display: flex; flex-wrap: wrap; gap: 8px 16px; font-family: var(--font-mono); font-size: 0.75rem; color: var(--faint); margin-bottom: 8px; }
.hero-meta a { color: var(--accent); text-decoration: none; }

.panel-card {
  background: linear-gradient(180deg, oklch(0.165 0.018 210), var(--surface));
  border: 1px solid var(--line); border-radius: 14px; box-shadow: var(--shadow);
  padding: 22px 24px 6px; margin-top: 18px; position: relative; overflow: hidden;
}
.panel-card::before {
  content: ""; position: absolute; left: 0; right: 0; top: 0; height: 1px;
  background: linear-gradient(90deg, transparent, var(--primary), var(--accent), transparent);
  background-size: 200% 100%; opacity: 0.55; animation: shimmer-line 5s linear infinite;
}
@keyframes shimmer-line { 0% { background-position: 0% 50%; } 100% { background-position: -200% 50%; } }
.step-label { font-family: var(--font-mono); font-size: 0.7rem; letter-spacing: 0.14em; text-transform: uppercase; color: var(--primary); margin-bottom: 8px; }
.panel-card h2 { margin: 0; font-size: 1.2rem; letter-spacing: -0.02em; font-weight: 600; }
.panel-card p.desc { margin: 6px 0 4px; color: var(--muted); font-size: 0.9rem; max-width: 60ch; }
...
[data-testid="stButton"] button:hover, [data-testid="stFormSubmitButton"] button:hover {
  transform: translateY(-1px); box-shadow: 0 10px 34px var(--primary-glow) !important;
}
...
.status-busy { color: var(--primary); background: var(--primary-soft); border-color: oklch(0.72 0.09 210 / 0.25); }
...
.module-row { display: flex; flex-wrap: wrap; gap: 8px; margin: 10px 0 16px; }
.pill {
...
.examples { display: flex; flex-wrap: wrap; gap: 8px; margin: 4px 0 16px; }
.example-chip {
...
.answer {
  margin: 12px 0 20px; background: oklch(0.1 0.01 210); border: 1px solid var(--line); border-left: 3px solid var(--primary);
...
<div class="panel-card">
  ...
  <h2>Add your Groq API key</h2>
  <p class="desc">This demo doesn't ship a shared key — bring your own, free, from
...
<div class="panel-card">
  ...
  <p class="desc">Clone, filter source files, chunk at 500 characters, embed with MiniLM, and index with FAISS IndexFlatL2.</p>
</div>
""",
    unsafe_allow_html=True,
)
    ...
    if not repo_url.strip():
        ...
        with st.spinner("Cloning, chunking, and building the vector index… large repos take a few minutes."):
            try:
                ...
                st.markdown(f'<div class="status status-err">✗ {exc}</div>', unsafe_allow_html=True)
            except CloneTimeoutError as exc:
                st.markdown(f'<div class="status status-err">✗ {exc}</div>', unsafe_allow_html=True)
            except Exception as exc:
                st.markdown(f'<div class="status status-err">✗ Analyze failed: {exc}</div>', unsafe_allow_html=True)
...
if st.session_state.session:
    ...
    if summary.get("main_modules"):
        pills = "".join(f'<span class="pill">{m}</span>' for m in summary["main_modules"])
...
<div class="panel-card">
  ...
  <h2>Ask the AI</h2>
  <p class="desc">Retrieve the top-5 nearest code chunks, then generate an explanation with Groq Llama 3.3 70B.</p>
</div>
""",
    unsafe_allow_html=True,
)
    ...
    if not st.session_state.groq_api_key:
        ...
        with st.spinner("Retrieving relevant chunks and generating an answer…"):
            try:
                ...
                st.markdown(f'<div class="status status-err">✗ Ask failed: {exc}</div>', unsafe_allow_html=True)
